    ...
    <div>
        <div>
            <div v-for="item in RenderObjectArray" :key="item.index">
                <div v-if="item.type === 'text'" class="flex items-start gap-2.5">
                    ...
                    <div
                        ...
                        bg-gradient-to-r from-finnopurple to-finnoblue rounded-e-xl rounded-es-xl dark:bg-gray-700">
                        <div class="flex items-center space-x-2 rtl:space-x-reverse">
                            <span class="text-sm font-semibold text-gray-900 dark:text-white">Charlie</span>
                        </div>
                        <p class="text-sm font-normal py-2.5 text-gray-900 dark:text-white">{{ item.value }}</p>
                    </div>
                </div>
                <div v-else-if="item.type === 'render'" class="flex items-start gap-2.5">
                    <div
                        class="flex flex-col w-full max-w-[800px] leading-1.5 p-4 border-gray-200 bg-gray-100 rounded-e-xl rounded-es-xl dark:bg-gray-700">
                        <p class="text-sm font-normal py-2.5 text-gray-900 dark:text-white">{{ item.value }}</p>
                    </div>
                </div>
            </div>
