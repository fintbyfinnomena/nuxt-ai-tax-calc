    ...
    methods: {
        UnpackPayload() {
            ...
            fundArray.forEach(fund => {
                ...
                let Amount = parseInt(Percentage * parseInt(this.taxInfoStore.desiredAmount))
                let fundObj = {
                    amount: Amount,
        ...
        async Invest() {
            this.UnpackPayload();
            let res = await fetch(`${this.config.public.url.serviceUrl}/api/v1/customer/batch-order/`, {
                method: "POST",
...
<template>
    <AlertDialog>
        ...
        <AlertDialogContent>
            <AlertDialogHeader>
                <AlertDialogTitle>คุณกำลังจะถูกนำทางไปหน้าซื้อกองทุนลดหย่อนภาษี</AlertDialogTitle>
                <AlertDialogDescription>
